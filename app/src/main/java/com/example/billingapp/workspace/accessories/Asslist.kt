...
class Asslist {
        ...
        if (viewModel.loading) {
            ...
            LazyColumn {
                items(viewModel.assItems) { item ->
                  AssItemRow(item = item, navController = navController)
                }
            }
        }
    }
    ...
    @Composable
    fun AssItemRow(item: assdata, navController: NavHostController) {
        Card(colors= CardColors(containerColor = Color(0xFFFFBBA6)
            ...
            modifier = Modifier
                .fillMaxWidth()
                .padding(7.5.dp)
                .clickable { navController.navigate("accessories/${item.name}") }
        ) {
            Column(
                modifier = Modifier
                    .padding(16.dp)
